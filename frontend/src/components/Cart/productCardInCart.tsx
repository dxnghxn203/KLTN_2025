...
const ProductCardInCart: React.FC<ProductCardInCartProps> = ({
  ...
  const renderRatingStars = (rating: number) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        stars.push(
          <BsStarFill key={`star-${i}`} className="text-yellow-400" />
        );
      } else if (i === fullStars && hasHalfStar) {
        stars.push(
          <BsStarHalf key={`star-${i}`} className="text-yellow-400" />
        );
      } else {
  ...
  return (
    <div
      className={clsx(
        ...
        isHovered
          ? "shadow-lg border-[#0053E2] transform -translate-y-1"
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-gray-50">
        ...
        {product.is_verified && (
          <div className="absolute top-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded shadow-sm flex items-center gap-1">
            <MdVerified size={12} />
      ...
      <div className="p-3">
        ...
        <div className="mt-3 relative" ref={dropdownRef}>
          ...
          {/* Custom dropdown */}
          {isDropdownOpen && (
            <div className="absolute z-10 w-full mt-1 bg-white border rounded-md shadow-lg max-h-60 overflow-auto py-1">
              {product.prices &&
                product.prices.map((priceOption: any) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      selectedUnit === priceOption.price_id
