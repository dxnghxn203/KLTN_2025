...
const SimilarProductsList = ({
    ...
    const fetchSimilarProducts = async () => {
        try {
            ...
            const response = await getSimilarProducts(productId, pageSize, currentPage);
                            ...
                            <div
                                ...
                                {similarProducts.map((product) => (
                                    <div key={product.product_id} className="h-full">
                                        <ProductCardInCart
                                            product={product}
                                            selectedUnit={selectedPrices[product.product_id] || (product.prices && product.prices[0]?.price_id)}
                                            onUnitChange={(priceId) => handleUnitChange(product.product_id, priceId)}
                            ...
                            <div className="rounded-full bg-gray-100 p-6 mb-5">
                                <svg className="w-12 h-12 text-gray-400" fill="none" stroke="currentColor"
                                    ...
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                                          d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M9 16h6M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                            </div>
                            <h3 className="text-lg font-medium text-gray-900 mb-1">Không tìm thấy sản phẩm tương tự</h3>
                            <p className="text-gray-500 max-w-md">
                                Hiện tại không có sản phẩm tương tự nào được tìm thấy. Vui lòng thử lại sau hoặc tìm
                                kiếm sản phẩm khác.
                            </p>
                        </div>
                    )}
