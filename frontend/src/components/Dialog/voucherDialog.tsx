...
const VoucherDialog: React.FC<VoucherDialogProps> = ({
  ...
  useEffect(() => {
    if (totalAmount && setVouchers) {
      ...
      if (
        ...
        selectedVoucherOrder.min_order_value > totalAmount
      ) {
  ...
  useEffect(() => {
    if (voucherErrors.length > 0 && setVouchers) {
      ...
      if (
        selectedVoucher &&
        voucherErrors.some(
          (error) => error.voucher_id === selectedVoucher.voucher_id
        )
      ) {
        newSelectedVoucher = null;
      }
      ...
      if (
        selectedVoucherOrder &&
        voucherErrors.some(
          (error) => error.voucher_id === selectedVoucherOrder.voucher_id
        )
      ) {
        newSelectedVoucherOrder = null;
      }
      ...
      if (
        ...
        newSelectedVoucherOrder !== selectedVoucherOrder
      ) {
        setSelectedVoucher(newSelectedVoucher);
        setSelectedVoucherOrder(newSelectedVoucherOrder);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white w-full max-w-lg max-h-[100vh] rounded-lg flex flex-col items-center relative overflow-hidden px-6 py-4">
        <button
          onClick={onClose}
        ...
        {!orderCheck && (
          <div className="w-full bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-md mb-4">
            <p className="text-center font-medium text-sm">
              Bạn hãy chọn sản phẩm để tiếp tục
            </p>
          </div>
        )}
                ...
                voucher.voucher_type === "delivery" && (
                  <label
                    ...
                    className={`flex justify-between items-center border rounded-lg mb-2 ${
                      isVoucherError(voucher.voucher_id) ||
                      hasUserUsedVoucher(voucher)
                  ...
                  >
                    <div className="flex">
                      <div className="relative rang-cua-left w-24 bg-[#26A999] text-white text-center font-bold p-2 rounded-l-lg">
                        FREE
                        <br />
                        SHIP
                        <p className="text-[10px] mt-1">Toàn Ngành Hàng</p>
                        {isSingleUseVoucher(voucher) && (
                          <div className="absolute -top-1 -right-1 bg-red-500 text-white text-[8px] px-1 py-0.5 rounded-full">
                            Dùng 1 lần
                          </div>
                        )}
                      </div>
                      <div className="flex flex-col px-3 py-1 justify-center">
                        <div className="flex justify-between items-center">
                          ...
                          {isSingleUseVoucher(voucher) && (
                            <span className="ml-2 text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-medium">
                              Chỉ dùng 1 lần
                            </span>
                          )}
                        </div>
                        ...
                        <p className="text-xs text-gray-600">
                          {highlightPercentage(
                            truncateText(voucher.description, 50)
                          )}
                        </p>
            ...
            {orderVouchersToShow.map(
              (voucher: any, index: number) =>
                voucher.voucher_type === "order" && (
                  ...
                  >
                    <div className="flex">
                      <div className="relative rang-cua-left w-24 bg-[#EA4B2A] text-white text-center font-bold p-2 rounded-l-lg rang-cua-left">
                        ĐƠN
                        <br />
                        HÀNG
                        <p className="text-[10px] mt-1">Toàn Ngành Hàng</p>
                    ...
                    <div
                      onClick={() => {
                        if (
                          ...
                          if (
                            selectedVoucherOrder?.voucher_id ===
                            voucher.voucher_id
                          ) {
