...
const TableBrand = ({ allBrandAdmin }: TableBrandProps) => {
  ...
  // Phân trang
  const totalProducts = allBrandAdmin ? allBrandAdmin.length : 0;
  const indexOfLastProduct = currentPage * productsPerPage;
  const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
  const currentProducts = allBrandAdmin?.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
          ...
          <table className="w-full table-auto border-collapse">
            <thead className="text-left text-[#1E4DB7] font-bold border-b border-gray-200 bg-[#F0F3FD]">
              <tr className="uppercase text-sm">
                <th className="p-4 text-center">Mã Brand</th>
                <th className="py-4 ">Loại Brand</th>
                <th className="py-4 text-center">Mô tả</th>
                <th className="py-4 text-center">Giảm giá</th>
                <th className="py-4 text-center">Giá trị Brand</th>
