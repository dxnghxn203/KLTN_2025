...
const CreateSingleProduct = () => {
    ...
    useEffect(() => {
        if (!productId || !allProductAdmin || allProductAdmin.length === 0 || !categoryAdmin) return;

        const product = allProductAdmin.find((item: any) => item.product_id === productId);
        if (!product) {
            toast.showToast("Không tìm thấy sản phẩm", "error");
            return;
        }

        // Update form data with product data
        setFormData({
    ...
    useEffect(() => {
        if (formData.selectedCategories.child && availableChildCategories.length > 0) {
            const childCat = availableChildCategories.find(
                (cat: any) => cat.child_category_id === formData.selectedCategories.child
            );
            if (childCat) {
                setFormData(prev => ({
                    ...prev,
    ...
    useEffect(() => {
        if (formData.product_name) {
            setFormData(prev => ({...prev, slug: generateSlug(formData.product_name)}));
        }
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.product_name) newErrors.product_name = "Vui lòng điền tên sản phẩm";
        ...
        if (!formData.uses || formData.uses.trim() === "" || formData.uses === "<p><br></p>") {
    ...
    const submitProduct = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        formDataObj.set("ingredients", JSON.stringify({ingredients: formData.ingredients}));
        ...
        formDataObj.set("prescription_required", JSON.stringify(formData.prescription_required));
                ...
                <div className="flex gap-4 h-full">
                    <div className="w-2/3 flex flex-col space-y-5">
                        ...
                        <ManufacturerForm
                            ...
                            brand={formData.brand}
                            updateBrand={(brand) => setFormData(prev => ({...prev, brand}))}
                            errors={errors}
                            hasError={hasError}
                            isViewOnly={isViewOnly}
                        />
