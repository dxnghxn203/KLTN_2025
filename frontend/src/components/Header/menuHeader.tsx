...
export default function MenuHeader() {
    const {allCategory, fetchAllCategory, mainCategory, fetchMainCategory, fetchGetCategoryForMenu} =
        useCategory();
    ...
    const handleMouseEnter = async (category: any) => {
        if (!subCategoryMap[category.main_category_slug] || subCategoryMap[category.main_category_slug].length === 0) {
            ...
            <ul className="flex justify-center space-x-10 font-semibold text-sm ">
                {visibleCategories.map((category: any) => (
                    <div key={category.main_category_slug}>
                        ...
                        >
                            ...
                            <div
                                className="absolute left-0 top-full mt-1 bg-white shadow rounded-lg hidden group-hover:block z-10">
                                <ul className="text-sm py-2">
                                    ...
                                    <li className="flex gap-2 flex-col whitespace-nowrap font-medium">
                                        {subCategoryMap[category.main_category_slug]?.map((subCategory: any) => (
                                            <Link
                ...
                <li className="relative group text-white uppercase">
                    ...
                    {extraCategories.length > 0 && (
                        <div
                            className="absolute left-0 top-full mt-0 bg-white border border-gray-200 shadow-lg rounded-lg hidden group-hover:block z-10 w-[200px]">
                            <ul className="text-sm text-gray-700 py-2">
