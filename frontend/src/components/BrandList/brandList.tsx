...
const BrandItem: React.FC<BrandProps> = ({name, logo}) => (
    ...
    >
        <div
            className="flex flex-col justify-center items-center px-7 rounded-full bg-neutral-100 h-[170px] w-[170px] max-md:px-5">
            <img
                loading="lazy"
                src={logo}
                alt={`${name} logo`}
    ...
    return (
        <div className="flex flex-col w-full max-md:px-5 max-md:max-w-full">
            <Link href="/thuong-hieu">
                <div
                    className="px-6 flex flex-wrap gap-5 justify-between items-start w-full text-black max-md:max-w-full">
            ...
            <div className="flex gap-5 justify-around items-center py-0.5 mt-7 w-full max-md:flex-wrap">
                {getAllBrandsUser && getAllBrandsUser.slice(0, 6).map((brand: any, index: any) => (
